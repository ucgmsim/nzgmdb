...
- **Site data and waveform preprocessing**
- **Event metadata, classification, and distance calculations**

Both tracks eventually converge to compute Intensity Measures (IMs), merge data into a full database, and apply quality filtering.

Below is a breakdown of each stream and the merged steps.

---

## 🛰️ Site Information Stream

### **Fetch Site Info**
Gathers all stations in the NZ network, including Vs30 and basin metadata.

### **Waveform Extraction**
Downloads MSEED waveform files from GeoNet.

### **Phase Arrival Detection**
Detects P- and S-wave arrival times using a custom picker and GeoNet picks.

### **Signal-to-Noise Ratio (SNR)**
Calculates SNR from waveforms and prepares for frequency-domain analysis.

### **Fmax Calculation**
Determines usable bandwidth limit (Fmax) from SNR data.

### **Ground Motion Classification (GMC)**
Classifies records (machine learning-based) to generate Fmin and quality metadata.

### **Process Waveforms**
Applies filters, trims data based on GMC, and converts MSEED to text.
...
Refines event locations based on improved location algorithms.

### **Classify Tectonic Type**
Adds tectonic region classification for each event.

### **CCLD Calculation**
Computes centroid and closest distance metrics.

### **Compute Distances**
Uses updated fault geometry to derive Rrup and other distance measures.

### **Aftershock Classification**
Determines whether an event is an aftershock (used for filtering).
...
Produces a cleaned flatfile with well-defined IMs.

---

## 🗂️ Final Outputs

- Cleaned and filtered flatfiles  
- Component-separated IM tables  
- Dropbox upload-ready packages *(optional: Upload Step)*
# File Structure
