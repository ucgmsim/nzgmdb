# Added SNR, whether the data was filtered for a better SNR, and amplitude time to
...
def glitch_catcher(tr: Trace, window_length: float, threshold: float):
    """
    ...
    diff_moving = np.concatenate(
        [np.array([diff_moving.mean()] * (len(diffs) - len(diff_moving))), diff_moving]
    )
...
def _snr(tr, noise_window, signal_window):
    ...
    """
    noise_amp = _rms(tr.slice(starttime=noise_window[0], endtime=noise_window[1]).data)
    ...
    try:
        signal_amp = tr.slice(
            starttime=signal_window[0], endtime=signal_window[1]
        ).data.max()
...
def _max_p2t(data, delta, return_peak_trough=False):
    ...
    if len(turning_points) >= 1:
        ...
        half_periods = np.empty(
            [len(turning_points) - 1],
        )
    else:
        print(
            "Turning points has length: "
            + str(len(turning_points))
            + " data have length: "
            + str(len(data))
        )
        return 0.0, 0.0, 0.0
    ...
    peak = max(t[0] for t in turning_points[max_position : max_position + 2])
    trough = min(t[0] for t in turning_points[max_position : max_position + 2])
        ...
        for event in cat:
            ...
            if pref_mag_type.lower() == "m":
                ...
                mb_mag = [
                    ...
                    if mag.magnitude_type.lower() == "mb"
                ]
                if mb_mag:
                    mb_mag = mb_mag[
                        np.array([mb_mag.station_count for mb_mag in mb_mag]).argmax()
                    ]
                ml_loc_mag = [
                    mag
                    for mag in event.magnitudes
                    if mag.magnitude_type.lower() == "ml"
                ]
                if ml_loc_mag:
                    ml_loc_mag = ml_loc_mag[
                        np.array(
                            [ml_loc_mag.station_count for ml_loc_mag in ml_loc_mag]
                        ).argmax()
                    ]
                mlv_loc_mag = [
                    mag
                    for mag in event.magnitudes
                    if mag.magnitude_type.lower() == "mlv"
                ]
                if mlv_loc_mag:
                    mlv_loc_mag = mlv_loc_mag[
                        np.array(
                            [mlv_loc_mag.station_count for mlv_loc_mag in mlv_loc_mag]
                        ).argmax()
                    ]
                if mb_mag:
                        ...
                        if len(loc_mag) > 0:
                            if loc_mag.station_count > mb_mag.station_count:
                                ...
                                print(
                                    eventid
                                    + "mb count: "
                                    + str(mb_mag.station_count)
                                    + " ml count: "
                                    + str(loc_mag.station_count)
                                )
                        else:
                            loc_mag = mb_mag
                            pref_mag_type = "Mb"
                ...
                pref_mag_nmag = len(
                    event.preferred_magnitude().station_magnitude_contributions
                )
                pref_mag_sta_ids = (
                    event.preferred_magnitude().station_magnitude_contributions
                )
            ...
            pick_data.extend(
                [
                    ...
                    if not any(
                        ...
                        if amp.waveform_id.station_code in pick
                    )
                ]
            )
            pick_data = pd.DataFrame(
                pick_data, columns=["net", "sta", "loc", "chan", "time", "phase_hint"]
            )
                ...
                if (
                    ...
                    if len(chan) < 3:
                        try:
                            st = client.get_waveforms(
                                net, sta, loc, chan + "?", windowStart, windowEnd
                            )
                            st = st.merge()
                        except:
                            continue
                        ...
                        try:
                            st = client.get_waveforms(
                                net, sta, loc, chan[0:2] + "?", windowStart, windowEnd
                            )
                            st = st.merge()
                        except:
                            continue
                    ...
                    for tr in st:
                        ...
                        if sta_mag:
                            ...
                            sta_mag_mag = None
                            sta_mag_type = (
                                pref_mag_type  # Set to event preferred magnitude type
                            )
                            amp = None
                        if amp:
                            amp = amp[0]
                        ...
                        if (
                            len(
                                trim_tr.slice(
                                    starttime=noise_window[0], endtime=noise_window[1]
                                ).data
                            )
                            > 0
                        ):
                            ...
                            snr = abs(
                                trim_tr.max()
                                / np.sqrt(np.mean(np.square(trim_tr.data)))
                            )
                        if snr < 3:
                            # Not a high enough SNR
                            print(
                                f"SNR for {tr.id} not high enough, attempting to filter data"
                            )

                            tr.filter(
                                "highpass", freq=lowcut, corners=corners, zerophase=True
                            )
                            trim_tr = tr.copy().trim(tr_starttime, tr_endtime)
                            if (
                                len(
                                    trim_tr.slice(
                                        starttime=noise_window[0],
                                        endtime=noise_window[1],
                                    ).data
                                )
                                > 0
                            ):
                        ...
                        if filt_data:
                            sos = iirfilter(
                                corners * 2,
                                [lowcut / (0.5 * tr.stats.sampling_rate)],
                                btype="highpass",
                                ftype="butter",
                                output="sos",
                            )
                            _, gain = sosfreqz(
                                sos, worN=[1 / period], fs=tr.stats.sampling_rate
                            )
                        ...
                        if r_epi:
                            ...
                            A = amplitude
                            ML_i = (
                                np.log10(A) + np.log10(R) + 0.0029 * R
                            )  # Robinson, 1987, add K for station correction
                            ...
                            NZlogA0R = (
                                ...
                                + ((2.38e-3) * H40)
                                + corr
                            )  # Corrected Ml for NZ according to Rhoades et al., 2021
            ...
            if pref_mag_type.lower() == "ml" or pref_mag_type.lower() == "mlv":
                ...
                if (
                    len(sta_mag_df[sta_mag_df.mag_corr.isnull() == False].sta.unique())
                    > 1
                ):
                    ...
                    if (mag_data.mag_corr.isnull() == False).sum() < 2:
                        mag_data = sta_mag_df[
                            sta_mag_df.chan.str.endswith(("E", "N", "1", "2"), na=False)
                        ]
                        if (
                            len(
                                mag_data[
                                    mag_data.mag_corr.isnull() == False
                                ].sta.unique()
                            )
                            < 2
                        ):
                    ...
                    if mag_type[0:3] == "cMl":
                        ...
                        IQR_mags = mag_data[
                            ~(
                                (mag_data.mag_corr < lowqe_bound)
                                | (mag_data.mag_corr > upper_bound)
                            )
                        ]
                        # Run a check for anomalous station magnitudes (+ or - 2 from original value)
                        # 						if len(IQR_mags.sta.unique()) <= 3:
                        IQR_mags = IQR_mags[
                            (IQR_mags.mag_corr <= event.preferred_magnitude().mag + 2)
                            & (IQR_mags.mag_corr >= event.preferred_magnitude().mag - 2)
                        ]
                        ...
                        event_line.append(
                            [
                                ...
                                event.preferred_magnitude().mag_errors.uncertainty,
                                ev_ndef,
                                ev_nsta,
                                nmag,
                                std,
            ...
            for i, arrival in enumerate(arrivals):
                ...
                pick = [
                    pick for pick in event.picks if pick.resource_id == arrival.pick_id
                ][0]
                ...
                phase_line.append(
                    [arid, arr_datetime, net, sta, loc, chan, phase, arr_t_res, eventid]
                )
...
sta_corr = pd.read_csv("sta_corr.csv")
...
date_range = pd.date_range(start="2015-05", end="2015-06", freq="MS")

for idx, date in enumerate(date_range[0:-1]):
    year = date.year
    month = date.month
    geonet_sub_mask = (geonet.origintime >= date) & (
        geonet.origintime < date_range[idx + 1]
    )
    ...
    delayed_results = []
    for i in event_list:
        result = dask.delayed(convert_eq)(
            i,
            client_NZ,
            client_IU,
            station_df,
...
#
events_df = pd.concat(
    [pd.read_csv(f, low_memory=False) for f in glob.glob(directory + "events_*.csv")]
)
mags_df = pd.concat(
    [pd.read_csv(f, low_memory=False) for f in glob.glob(directory + "mags_*.csv")]
)
arrivals_df = pd.concat(
    [pd.read_csv(f, low_memory=False) for f in glob.glob(directory + "arrivals_*.csv")]
)
