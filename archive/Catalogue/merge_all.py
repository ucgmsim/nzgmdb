...
prop_df = pd.read_csv(
    directory + "propagation_path_table_complete.csv", low_memory=False
)
...
indexNames = prop_df_sub[
    (prop_df_sub["net"] == "AU") & (prop_df_sub["sta"] == "ARPS")
].index
...
gm_im_df_flat.rename(
    columns={"lat": "ev_lat", "lon": "ev_lon", "depth": "ev_depth"}, inplace=True
)
...
gm_im_df_flat = (
    gm_im_df_flat.set_index(gm_im_df_flat["evid"] + gm_im_df_flat["sta"])
    .join(
        prop_df_sub.set_index(prop_df_sub["evid"] + prop_df_sub["sta"])[
            ["r_epi", "r_hyp", "r_jb", "r_rup", "r_x", "r_y", "r_tvz", "r_xvf"]
        ],
        how="left",
    )
...
df_rotd50_flat = df_rotd50_flat.drop(
    ["score_mean_Z", "fmin_mean_Z", "fmax_mean_Z", "multi_mean_Z"], axis=1
)
df_rotd100_flat = df_rotd100_flat.drop(
    ["score_mean_Z", "fmin_mean_Z", "fmax_mean_Z", "multi_mean_Z"], axis=1
)
...
df_rotd100_flat.to_csv(
    directory + "ground_motion_im_table_rotd100_flat.csv", index=False
)
