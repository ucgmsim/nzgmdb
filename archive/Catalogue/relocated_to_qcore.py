...
def merge_reloc(directory):
    ...
    relocated_event_cat = pd.read_csv(
        directory + "relocated_earthquake_source_table.csv", low_memory=False
    )
    relocated_event_cat["datetime"] = pd.to_datetime(
        relocated_event_cat.datetime
    ...
    relocated_mag_df = pd.read_csv(
        directory + "relocated_station_magnitude_table.csv", low_memory=False
    )
    ...
    ISC_relocated_event_cat = pd.read_csv(
        directory + "ISC_relocated_earthquake_source_table.csv", low_memory=False
    )
    ISC_relocated_event_cat["datetime"] = pd.to_datetime(
        ISC_relocated_event_cat.datetime
    ).astype("datetime64[ns]")
    ...
    ISC_relocated_mag_df = pd.read_csv(
        directory + "ISC_relocated_station_magnitude_table.csv", low_memory=False
    )
...
def convert_eq(
    ...
    try:
        ...
        for sta_index, sta_mag in sta_mags.iterrows():
            ...
            if (
                sta_mag.mag_type.lower() == "ml" or sta_mag.mag_type.lower() == "mlv"
            ) and sta_mag.isnull().mag_corr == False:
                ...
                if len(sta_info) > 0:
                    ...
                    NZlogA0R = (
                        ...
                        + ((2.38e-3) * H40)
                        + corr
                    )  # Corrected Ml for NZ according to Rhoades et al., 2020
        ...
        if (
            ev.mag_type.lower() == "ml"
            or ev.mag_type.lower() == "mlv"
            or ev.mag_type.lower() == "cml"
        ):
            ...
            if len(sta_mag_df[sta_mag_df.mag_corr.isnull() == False].sta.unique()) > 1:
                ...
                if (mag_data.mag_corr.isnull() == False).sum() < 2:
                    ...
                    if (
                        len(mag_data[mag_data.mag_corr.isnull() == False].sta.unique())
                        < 2
                    ):
                        mag_data = sta_mag_df
                ...
                if mag_type[0:3] == "cMl":
                    ...
                    IQR_mags = IQR_mags[
                        (IQR_mags.mag_corr <= ev.mag_orig + 2)
                        & (IQR_mags.mag_corr >= ev.mag_orig - 2)
                    ]
...
event_df = pd.read_csv(
    "/Volumes/SeaJade 2 Backup/NZ/NZ_EQ_Catalog/martins_new.csv", low_memory=False
)
...
for idx, date in enumerate(date_range[0:-1]):
    year = date.year
    month = date.month
    event_df_sub_mask = (event_df.datetime >= date) & (
        event_df.datetime < date_range[idx + 1]
    )
    event_df_sub = event_df.copy()[event_df_sub_mask].reset_index(drop=True)
    if len(event_df_sub) > 0 and np.any(
        np.isin(event_df_sub.evid.values, event_cat.evid.values)
    ):
        ...
        event_df_file = (
            directory + "relocated_events_" + str(year) + "_" + str(month) + ".csv"
        )
        mag_df_file = (
            directory + "relocated_mags_" + str(year) + "_" + str(month) + ".csv"
        )
...
events_df = pd.concat(
    [
        ...
        for f in glob.glob(directory + "relocated_events_*.csv")
    ]
)
mags_df = pd.concat(
    [
        pd.read_csv(f, low_memory=False)
        for f in glob.glob(directory + "relocated_mags_*.csv")
    ]
)
...
event_df["datetime"] = pd.to_datetime(
    event_df[["year", "month", "day", "hour", "minute", "second"]], errors="coerce"
)
...
for idx, date in enumerate(date_range[0:-1]):
    ...
    event_df_sub = event_df.copy()[event_df_sub_mask].reset_index(drop=True)
    event_df_sub = event_df_sub[
        (event_df_sub.depth_source == "ISC-EHB")
        | (event_df_sub.depth_source == "ISC-GEM")
    ]
    if len(event_df_sub) > 0 and np.any(
        np.isin(event_df_sub.evid.values, event_cat.evid.values)
    ):
        ...
        event_df_file = (
            directory + "ISC_relocated_events_" + str(year) + "_" + str(month) + ".csv"
        )
        mag_df_file = (
            directory + "ISC_relocated_mags_" + str(year) + "_" + str(month) + ".csv"
        )
...
events_df = pd.concat(
    [
        pd.read_csv(f, low_memory=False)
        for f in glob.glob(directory + "ISC_relocated_events_*.csv")
    ]
)
mags_df = pd.concat(
    [
        pd.read_csv(f, low_memory=False)
        for f in glob.glob(directory + "ISC_relocated_mags_*.csv")
    ]
)
