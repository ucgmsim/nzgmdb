...
search_dirs = ['mseed_4-10_2022_preferred','CPLB_mseed_4-10_preferred','mseed_4-4.5_preferred','mseed_4.5-5_preferred','mseed_5-6_preferred','mseed_6-10_preferred']
...
df = pd.read_csv(directory+'IM_catalogue/complete_ground_motion_im_catalogue_final.csv',low_memory=False)
df = df[df.component == 'rotd50'].reset_index(drop = True)
df['datetime'] = pd.to_datetime(df['datetime'])
eq_df = pd.read_csv(directory+'earthquake_source_table_complete.csv',low_memory=False)
...
for idx,date in enumerate(date_range[0:-1]):
	...
	df_out.to_csv(directory+'clip_table_'+str(year)+'_'+str(month)+'.csv',index=False)
	
df_out = pd.concat([pd.read_csv(f,low_memory=False) for f in glob.glob(directory+'clip_table*.csv')])
