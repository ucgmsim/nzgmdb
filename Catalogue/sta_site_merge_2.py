...
# sc_df = pd.read_excel('/Volumes/SeaJade 2 Backup/NZ/NZ_EQ_Catalog/site_info/Geonet  Metadata  Summary_v1.4.xlsx')
sc_df = pd.read_csv("../Geonet  Metadata  Summary_v1.4.csv")
sc_df.rename(
    columns={
        "Name": "sta",
        "Lat": "lat",
...
merged_df = (
    sc_df.set_index(sc_df.sta)
    .join(sta_df.set_index(sta_df.sta)[["net", "elev"]], how="left")
    .reset_index(drop=True)
)
merged_df[["site_domain_no", "site_domain_name", "site_domain_type"]] = merged_df.apply(
...
merged_final_df["Z2.5"] = merged_final_df["Z2.5"] / 1000

drop_index = merged_final_df[
    (merged_final_df.net == "AU") & (merged_final_df.sta == "ARPS")
].index

merged_final_df.drop(index=drop_index, inplace=True)

merged_final_df.to_csv(
    "/Volumes/SeaJade 2 Backup/NZ/NZ_EQ_Catalog/converted_output/site_table_response.csv",
    index=False,
)
