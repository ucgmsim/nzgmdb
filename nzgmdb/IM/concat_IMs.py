...
files = glob.glob(
    directory + "/ground_motion*final.csv"
)  # Check to make sure this doesn't merge in an already merged file
...
gm_im_dfs = gm_im_dfs[
    (gm_im_dfs.score_mean_X >= 0.5)
    & (gm_im_dfs.score_mean_Y >= 0.5)
    & (gm_im_dfs.score_mean_Z >= 0.5)
]
# Filter by multi mean
gm_im_dfs = gm_im_dfs[
    (gm_im_dfs.multi_mean_X <= 0.2)
    & (gm_im_dfs.multi_mean_Y <= 0.2)
    & (gm_im_dfs.multi_mean_Z <= 0.2)
]
# Filter by fmin mean
gm_im_dfs = gm_im_dfs[
    (gm_im_dfs.fmin_mean_X <= 2)
    & (gm_im_dfs.fmin_mean_Y <= 2)
    & (gm_im_dfs.fmin_mean_Z <= 2)
]

# Filter for Ds595
sub = gm_im_dfs[
    (gm_im_dfs.component == "ver")
    | (gm_im_dfs.component == "000")
    | (gm_im_dfs.component == "090")
...
gm_im_dfs = (
    gm_im_dfs.set_index(gm_im_dfs.evid + "_" + gm_im_dfs.sta + "_" + gm_im_dfs.chan)
    .merge(fmax_df.set_index("ev_sta"), left_index=True, right_index=True, how="left")
    .reset_index(drop=True)
)
gm_im_dfs[["fmax_mean_X", "fmax_mean_Y", "fmax_mean_Z"]] = gm_im_dfs[
    ["fmax_090", "fmax_000", "fmax_ver"]
].values
gm_im_dfs.drop(columns=["fmax_090", "fmax_000", "fmax_ver"], inplace=True)
gm_im_dfs = gm_im_dfs[
    (gm_im_dfs.fmax_mean_X > 4.1)
    & (gm_im_dfs.fmax_mean_Y > 4.1)
    & (gm_im_dfs.fmax_mean_Z > 4.1)
].reset_index(drop=True)

# Merge clip data and filter by clip probability
gm_im_dfs = (
    gm_im_dfs.set_index(gm_im_dfs.evid + "_" + gm_im_dfs.sta)
    .merge(
        clip_df.set_index(clip_df.evid + "_" + clip_df.sta)[["clip_prob", "clipped"]],
        left_index=True,
        right_index=True,
        how="left",
    )
...
for event in unique_events:
    ...
    gmids = (
        ...
        + "gm"
        + (
            gm_im_dfs[gm_im_dfs.evid.values == event].reset_index().index.to_series()
            + 1
        ).astype("str")
    ).values
...
gm_im_dfs.to_csv(
    directory + "/complete_ground_motion_im_catalogue_final.csv", index=False
)
