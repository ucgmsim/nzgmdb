...
def geonet_rupture(file):
    ...
    columns = ['segment','northing','easting','depth','slip','strike','dip','rake','width',
        'length','rupture_time']
    df = pd.read_csv(file,
        comment='%',sep='\s+',header=None,names=columns)
...
#     olat, olon = -43.545, 172.69
    ...
    for segment in segments:
        ...
        qx, qy = ox + df_sub[df_sub.depth == dtop * 1000].easting.iloc[-1], oy + df_sub[df_sub.depth == dtop * 1000].northing.iloc[-1]
    ...
    #     vdip_dist = np.sin(np.deg2rad(dip)) * width
        ...
        srf_header.append({"nstrike": nstrike, "ndip": ndip, "strike": strike, "dip": dip, 
            "rake": rake, "length": plane_point_distance, "width": width})
...
def path_props(df,df_merged,df_station,taupo_polygon,cmt_df,domain_focal_df,srf_files,srfs,geonet_cmt_df,event_out_file,props_out_file):
    ...
    if mag != -9 and ~np.isnan(mag):
        ...
        #         if len(srf_header) > 1:
            ...
            # Search for GEONET rupture models from which to draw parameters
                ...
                f_type = 'geonet_rm'
                georup_file = georup_files[np.where(np.isin(georups,evid) == True)[0][0]]
...
event_df = pd.read_csv(directory+'output/earthquake_source_table_tectdomain.csv',low_memory=False)
...
georup_files = glob.glob(directory+'focal/rupture-models/**/*_seismic_model.txt',recursive=True)
georups = [os.path.basename(georup_file).split('_')[0] for georup_file in georup_files]

df_im = pd.read_csv(directory+'output/IM_catalogue/complete_ground_motion_im_catalogue_final.csv',low_memory=False)
...
tect_domain_points = pd.read_csv(directory+'TectonicDomains/tectonic_domain_polygon_points.csv',low_memory=False)
...
# 	for month in months:
    ...
    event_out_file = directory+'output/earthquake_source_table_complete_'+str(process_year)+'.csv'
    props_out_file = directory+'output/propagation_path_table_complete_'+str(process_year)+'.csv'
...
#                'z_tor', 'z_bor'])
        ...
        pandarallel.initialize(nb_workers=6,progress_bar=True) # Customize the number of parallel workers
        event_sub.parallel_apply(lambda x: path_props(x,df_merged_sub,df_station,taupo_polygon,cmt_df,domain_focal_df,srf_files,srfs,geonet_cmt_df,event_out_file,props_out_file),axis=1)
        print('')
        print('Completed processing for year '+str(process_year))
# 

events_out_final = pd.concat([pd.read_csv(f,low_memory=False) for f in glob.glob(directory+'output/earthquake_source_table_complete_*.csv')])
props_out_final = pd.concat([pd.read_csv(f,low_memory=False) for f in glob.glob(directory+'output/propagation_path_table_complete_*.csv')])
...
# 
events_out_final.to_csv(directory+'output/earthquake_source_table_complete.csv',index=False)
props_out_final.to_csv(directory+'output/propagation_path_table_complete.csv',index=False)
