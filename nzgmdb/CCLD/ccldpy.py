...
TECTONIC_MAPPING = {
    ...
    "Slab": "intraslab",
    "Interface": "interface",
    "Outer-rise": "intraslab",
    "Undetermined": "crustal",
}
...
def WellsCoppersmith1994(magnitude, eqType):
    ...
    if eqType in ["intraslab", "interface", "stable"]:
        raise ValueError(
            "eqType: WellsCoppersmith1994 cannot be used for subduction-type or stable-continental events ('intraslab', 'interface', or 'stable')"
        )
...
def Leonard2014(magnitude, mechanism, eqType):
    ...
    if eqType in ["intraslab", "interface"]:
        raise ValueError(
            "eqType: Leonard2014 cannot be used for subduction-type events ('intraslab' or 'interface')"
        )
        return (np.nan, np.nan, np.nan, np.nan)
...
def ThingbaijamEtAl2017(magnitude, mechanism, eqType):
    ...
    if eqType in ["intraslab", "stable"]:
        raise ValueError(
            "eqType: ThingbaijamEtAl2017 cannot be used for instra-slab or stable-continental events ('intraslab' or 'stable')"
        )
        return (np.nan, np.nan, np.nan, np.nan)
...
def ChiouYoungs2008(magnitude, mechanism, eqType, model):
    ...
    if eqType in ["intraslab", "interface", "stable"]:
        raise ValueError(
            "eqType: ChiouYoungs2008 cannot be used for subduction-type or stable-continental events ('intraslab', 'interface', or 'stable')"
        )
        return (np.nan, np.nan, np.nan, np.nan)
        ...
        if magnitude < 4.0:
            ...
            b = 3.097
            AR = 10 ** (
                (ass + anm * FNM + arv * FRV) * (magnitude - 4.0) ** b
...
def ContrerasEtAl2022(magnitude, eqType):
    ...
    if eqType in ["crustal", "stable"]:
        raise ValueError(
            "eqType: ContrerasEtAl2022 cannot be used for non-subduction-type events ('crustal' or 'stable')"
        )
        return (np.nan, np.nan, np.nan, np.nan)
        ...
        if magnitude > 7.25:
            AR = 10.0 ** (
                0.2759 * (magnitude - 7.25) + np.random.normal(0, 0.192)
            )  # std = 0.441 in ln unit
        else:
        ...
        if magnitude > 6.5:
            AR = 10.0 ** (
                0.0938 * (magnitude - 6.5) + np.random.normal(0, 0.164)
            )  # std = 0.378in ln unit
        else:
...
def get_hyp_down_dip_position(eqtype, region):
    ...
    iad = np.array(
        [0.00, 0.012, 0.051, 0.139, 0.294, 0.500, 0.706, 0.861, 0.949, 0.988, 1.00]
    )
    ied0 = np.array(
        [0.000, 0.024, 0.085, 0.206, 0.389, 0.599, 0.783, 0.906, 0.969, 0.993, 1.00]
    )
    ied1 = np.array(
        [0.0, 0.002, 0.012, 0.044, 0.121, 0.262, 0.460, 0.671, 0.843, 0.950, 1.00]
    )
    ied2 = np.array(
        [0.00, 0.013, 0.053, 0.143, 0.297, 0.500, 0.703, 0.857, 0.947, 0.987, 1.00]
    )
    if eqtype in ["crustal", "stable"]:
...
def get_hyp_along_strike_position(eqtype):
    ...
    hypxe = np.array(
        [0.000, 0.007, 0.034, 0.112, 0.272, 0.500, 0.728, 0.888, 0.966, 0.993, 1.00]
    )
    hypxa = np.array(
        [0.00, 0.015, 0.057, 0.148, 0.301, 0.500, 0.699, 0.852, 0.943, 0.985, 1.00]
    )
    if eqtype in ["crustal", "stable"]:
...
def check_input_arguments(
    eqType, method, nsims, strike, dip, rake, strike2, dip2, rake2
):
    # Check that required arguments are defined for each simulation method  -------------------------------------------------
    if method in ["A", "D"]:
        if None in np.array([strike, dip, rake]):
            raise ValueError(
                "'strike', 'dip', and 'rake' must be defined when using 'method' = '%s'"
                % method
            )
    if method == "B":
        if None in np.array([strike2, dip2, rake2]):
            raise ValueError(
                "'strike2', 'dip2', and 'rake2' must be defined when using 'method' = 'B'"
            )
    if method == "C":
        if None in np.array([strike, dip, rake, strike2, dip2, rake2]):
            raise ValueError(
                "'strike', 'dip', 'rake', 'strike2', 'dip2', and 'rake2' must be defined when using 'method' = 'C'"
            )

    # Check that number of simulations are compatible with sepcified eqType  ------------------------------------------------
    if (eqType == "crustal") & (nsims[6] > 0):
        raise ValueError(
            "nsims[6] = %i: ContrerasEtAl2022 cannot be used for 'crustal' events"
            % nsims[6]
        )
    elif eqType == "stable":
        if nsims[0] > 0:
            raise ValueError(
                "nsims[0] = %i: WellsCoppersmith1994 cannot be used for 'stable' events"
                % nsims[0]
            )
        elif nsims[2] > 0:
            raise ValueError(
                "nsims[2] = %i: ThingbaijamEtAl2017 cannot be used for 'stable' events"
                % nsims[2]
            )
        elif sum(nsims[3:6]) > 0:
            raise ValueError(
                "nsims[3:6] = [%i,%i,%i]: ChiouYoungs2008 cannot be used for 'stable' events"
                % (nsims[3], nsims[4], nsims[5])
            )
        elif nsims[6] > 0:
            raise ValueError(
                "nsims[6] = %i: ContrerasEtAl2022 cannot be used for 'stable' events"
                % nsims[6]
            )
    elif eqType in ["interface", "intraslab"]:
        if nsims[0] > 0:
            raise ValueError(
                "nsims[0] = %i: WellsCoppersmith1994 cannot be used for '%s' events"
                % (nsims[0], eqType)
            )
        elif nsims[1] > 0:
            raise ValueError(
                "nsims[1] = %i: Leonard2014 cannot be used for '%s' events"
                % (nsims[1], eqType)
            )
        elif sum(nsims[3:6]) > 0:
            raise ValueError(
                "nsims[3:6] = [%i,%i,%i]: ChiouYoungs2008 cannot be used for '%s' events"
                % (nsims[3], nsims[4], nsims[5], eqType)
            )

    # Check that there are only positive integers for the number of simulations  --------------------------------------------
    if np.any(np.array(nsims), where=np.array(nsims) < 0):
        raise ValueError(
            "nsims = %s: can not specify a negative number of simulatiuons for any scaling relationship"
            % str(list(nsims))
        )
    ...
    if sum(nsims) % 2 == 0:
        if eqType == "crustal":
            if nsims[0] > 0:
                ...
                raise ValueError(
                    "nsims = %s: must specify the number of simulations partitioned to each scaling relationship"
                    % str(list(nsims))
                )
        if eqType == "stable":
            if nsims[1] > 0:
...
def get_rupture_surface_simulations(
    eqid,
    eqType,
    region,
    elat,
    ...
    if method == "A":
        ...
        strikes = np.random.uniform(
            strike - 30.0, strike + 30.0, total_sims
        )  # +/- 30 degrees
        ...
        dips = np.where(
            dips > 89.9999999, 89.9999999, dips
        )  # dip <= 90 # cannot divide by zero
        rakes = np.repeat([rake], total_sims)
        mechanisms = v_get_mechanism_based_on_rake(rakes)
    elif method == "E":
    ...
    if nsims[0] > 0:
        ...
        relationships_WellsCoppersmith1994 = v_WellsCoppersmith1994(
            np.repeat(magnitude, nsim_WellsCoppersmith1994),
    ...
    if nsims[1] > 0:
        ...
        relationships_Leonard2014 = v_Leonard2014(
            np.repeat(magnitude, nsim_Leonard2014),
            mechanisms_Leonard2014,
            np.repeat(eqType, nsim_Leonard2014),
        )
    ...
    if nsims[2] > 0:
        ...
        mechanisms_ThingbaijamEtAl2017 = mechanisms[
            np.sum(nsims[:2]) : np.sum(nsims[:3])
        ]
        relationships_ThingbaijamEtAl2017 = v_ThingbaijamEtAl2017(
            np.repeat(magnitude, nsim_ThingbaijamEtAl2017),
            mechanisms_ThingbaijamEtAl2017,
            np.repeat(eqType, nsim_ThingbaijamEtAl2017),
        )
    ...
    if nsims[3] > 0:
        ...
        mechanisms_ChiouYoungs2008_WellsCoppersmith1994 = mechanisms[
            np.sum(nsims[:3]) : np.sum(nsims[:4])
        ]
        relationships_ChiouYoungs2008_WellsCoppersmith1994 = v_ChiouYoungs2008(
            np.repeat(magnitude, nsim_ChiouYoungs2008_WellsCoppersmith1994),
        ...
        areas_ChiouYoungs2008_WellsCoppersmith1994 = (
            relationships_ChiouYoungs2008_WellsCoppersmith1994[0]
        )
        aspect_ratios_ChiouYoungs2008_WellsCoppersmith1994 = (
            relationships_ChiouYoungs2008_WellsCoppersmith1994[1]
        )
        lengths_ChiouYoungs2008_WellsCoppersmith1994 = (
            relationships_ChiouYoungs2008_WellsCoppersmith1994[2]
        )
        widths_ChiouYoungs2008_WellsCoppersmith1994 = (
            relationships_ChiouYoungs2008_WellsCoppersmith1994[3]
        )
        models = np.concatenate(
            (
                models,
                np.repeat(
                    "ChiouYoungs2008_WellsCoppersmith1994",
                    nsim_ChiouYoungs2008_WellsCoppersmith1994,
                ),
            )
        )
    else:
    ...
    if nsims[4] > 0:
        ...
        mechanisms_ChiouYoungs2008_Leonard2014 = mechanisms[
            np.sum(nsims[:4]) : np.sum(nsims[:5])
        ]
        relationships_ChiouYoungs2008_Leonard2014 = v_ChiouYoungs2008(
            np.repeat(magnitude, nsim_ChiouYoungs2008_Leonard2014),
        ...
        aspect_ratios_ChiouYoungs2008_Leonard2014 = (
            relationships_ChiouYoungs2008_Leonard2014[1]
        )
        lengths_ChiouYoungs2008_Leonard2014 = relationships_ChiouYoungs2008_Leonard2014[
            2
        ]
        widths_ChiouYoungs2008_Leonard2014 = relationships_ChiouYoungs2008_Leonard2014[
            3
        ]
        models = np.concatenate(
            (
                models,
                np.repeat(
                    "ChiouYoungs2008_Leonard2014", nsim_ChiouYoungs2008_Leonard2014
                ),
            )
        )
    else:
        areas_ChiouYoungs2008_Leonard2014, aspect_ratios_ChiouYoungs2008_Leonard2014 = (
            np.array([]),
            np.array([]),
        )
        lengths_ChiouYoungs2008_Leonard2014, widths_ChiouYoungs2008_Leonard2014 = (
            np.array([]),
            np.array([]),
        )
    ...
    if nsims[5] > 0:
        ...
        mechanisms_ChiouYoungs2008_ThingbaijamEtAl2017 = mechanisms[
            np.sum(nsims[:5]) : np.sum(nsims[:6])
        ]
        relationships_ChiouYoungs2008_ThingbaijamEtAl2017 = v_ChiouYoungs2008(
            np.repeat(magnitude, nsim_ChiouYoungs2008_ThingbaijamEtAl2017),
            mechanisms_ChiouYoungs2008_ThingbaijamEtAl2017,
            np.repeat(eqType, nsim_ChiouYoungs2008_ThingbaijamEtAl2017),
            "ThingbaijamEtAl2017",
        )
        areas_ChiouYoungs2008_ThingbaijamEtAl2017 = (
            relationships_ChiouYoungs2008_ThingbaijamEtAl2017[0]
        )
        aspect_ratios_ChiouYoungs2008_ThingbaijamEtAl2017 = (
            relationships_ChiouYoungs2008_ThingbaijamEtAl2017[1]
        )
        lengths_ChiouYoungs2008_ThingbaijamEtAl2017 = (
            relationships_ChiouYoungs2008_ThingbaijamEtAl2017[2]
        )
        widths_ChiouYoungs2008_ThingbaijamEtAl2017 = (
            relationships_ChiouYoungs2008_ThingbaijamEtAl2017[3]
        )
        models = np.concatenate(
            (
                models,
                np.repeat(
                    "ChiouYoungs2008_ThingbaijamEtAl2017",
                    nsim_ChiouYoungs2008_ThingbaijamEtAl2017,
                ),
            )
        )
    else:
    ...
    if nsims[6] > 0:
        ...
        relationships_ContrerasEtAl2022 = v_ContrerasEtAl2022(
            np.repeat(magnitude, nsim_ContrerasEtAl2022),
            np.repeat(eqType, nsim_ContrerasEtAl2022),
        )
    ...
    areas = np.concatenate(
        (
            areas_WellsCoppersmith1994,
            areas_Leonard2014,
            areas_ThingbaijamEtAl2017,
            areas_ChiouYoungs2008_WellsCoppersmith1994,
            areas_ChiouYoungs2008_Leonard2014,
            areas_ChiouYoungs2008_ThingbaijamEtAl2017,
    ...
    aspect_ratios = np.concatenate(
        (
            aspect_ratios_WellsCoppersmith1994,
            aspect_ratios_Leonard2014,
            aspect_ratios_ThingbaijamEtAl2017,
            aspect_ratios_ChiouYoungs2008_WellsCoppersmith1994,
            aspect_ratios_ChiouYoungs2008_Leonard2014,
            aspect_ratios_ChiouYoungs2008_ThingbaijamEtAl2017,
    ...
    lengths = np.concatenate(
        (
            lengths_WellsCoppersmith1994,
            lengths_Leonard2014,
            lengths_ThingbaijamEtAl2017,
            lengths_ChiouYoungs2008_WellsCoppersmith1994,
            lengths_ChiouYoungs2008_Leonard2014,
            lengths_ChiouYoungs2008_ThingbaijamEtAl2017,
    ...
    widths = np.concatenate(
        (
            widths_WellsCoppersmith1994,
            widths_Leonard2014,
            widths_ThingbaijamEtAl2017,
            widths_ChiouYoungs2008_WellsCoppersmith1994,
            widths_ChiouYoungs2008_Leonard2014,
            widths_ChiouYoungs2008_ThingbaijamEtAl2017,
    ...
    down_dips = np.where(
        top_depths < 0.0, hypd / rwv, down_dips
    )  # make sure hypocenter is in the ground
    ...
    r = np.array(
        list(np.arange(2, 20, 2))
        + list(np.arange(25, 55, 5))
        + list(np.arange(60, 110, 10))
        + list(np.arange(125, 325, 25))
    )
    theta = np.pi * np.linspace(0, 360, 25)[:-1] / 180
    psx = np.multiply(
        np.repeat(r.reshape(1, len(r)), len(theta), axis=0),
        np.sin(theta.reshape(len(theta), 1)),
    ).flatten()
    psy = np.multiply(
        np.repeat(r.reshape(1, len(r)), len(theta), axis=0),
        np.cos(theta.reshape(len(theta), 1)),
    ).flatten()
    ...
    TRI_123 = np.array(
        [
            ...
            for x1, x2, x3, y1, y2, y3, z1, z2, z3 in zip(
                rpx1s, rpx2s, rpx3s, rpy1s, rpy2s, rpy3s, rpz1s, rpz2s, rpz3s
            )
        ]
    )
    TRI_234 = np.array(
        [
            [[x1, y1, z1], [x2, y2, z2], [x3, y3, z3]]
            for x1, x2, x3, y1, y2, y3, z1, z2, z3 in zip(
                rpx2s, rpx4s, rpx3s, rpy2s, rpy4s, rpy3s, rpz2s, rpz4s, rpz3s
            )
        ]
    )
...
def simulate_rupture_surface(
    ...
    (
        ...
        top_depths,
        bottom_depths,
    ) = get_rupture_surface_simulations(
        eqid,
        eqType,
        region,
        elat,
    ...
    SIMULATIONS = pd.DataFrame(
        {
            "Simulation": np.array(range(total_sims)) + 1,
            "EQID": np.repeat(eqid, total_sims),
            "Magnitude": np.repeat(magnitude, total_sims),
            "Hypocenter Longitude": np.repeat(elon, total_sims),
            "Hypocenter Latitude": np.repeat(elat, total_sims),
            "Hypocenter Depth (km)": np.repeat(hypd, total_sims),
            "Hypocenter Along-Strike Position": along_strikes,
            "Hypocenter Down-Dip Position": down_dips,
            "ULC Longitude": lon2s,
            "ULC Latitude": lat2s,
            "ULC Depth (km)": rpz2s,
            "URC Longitude": lon1s,
            ...
            "Rupture Length (km)": lengths,
            "Rupture Width (km)": widths,
            "Rupture Top Depth (km)": top_depths,
            "Rupture Bottom Depth (km)": bottom_depths,
        }
    )

    SELECTED = pd.DataFrame(
        {
            "Median Simulation": [median_simulation + 1],
            "EQID": [eqid],
            "Magnitude": [magnitude],
            "Hypocenter Longitude": [elon],
            "Hypocenter Latitude": [elat],
            "Hypocenter Depth (km)": [hypd],
            "Hypocenter Along-Strike Position": [along_strikes[median_simulation]],
            "Hypocenter Down-Dip Position": [down_dips[median_simulation]],
            "ULC Longitude": [lon2s[median_simulation]],
            "ULC Latitude": [lat2s[median_simulation]],
            "ULC Depth (km)": [rpz2s[median_simulation]],
            "URC Longitude": [lon1s[median_simulation]],
            "URC Latitude": [lat1s[median_simulation]],
            "URC Depth (km)": [rpz1s[median_simulation]],
            "LRC Longitude": [lon3s[median_simulation]],
            "LRC Latitude": [lat3s[median_simulation]],
            "LRC Depth (km)": [rpz3s[median_simulation]],
            "LLC Longitude": [lon4s[median_simulation]],
            "LLC Latitude": [lat4s[median_simulation]],
            "LLC Depth (km)": [rpz4s[median_simulation]],
            "Strike": [strikes[median_simulation]],
            "Dip": [dips[median_simulation]],
            "Rake": [rakes[median_simulation]],
            "Scaling Relation": [models[median_simulation]],
            "Area (km^2)": [areas[median_simulation]],
            "Average Area (km^2)": [areas_avg],
            "Area Standard Deviation (km^2)": [areas_std],
            "Aspect Ratio": [aspect_ratios[median_simulation]],
            "Average Aspect Ratio": [aspect_ratios_avg],
            "Aspect Ratio Standard Deviation": [aspect_ratios_std],
            "Length (km)": [lengths[median_simulation]],
            "Average Length (km)": [lengths_avg],
            "Length Standard Deviation (km)": [lengths_std],
            "Width (km)": [widths[median_simulation]],
            "Average Width (km)": [widths_avg],
            "Width Standard Deviation (km)": [widths_std],
            "Rupture Top Depth (km)": [top_depths[median_simulation]],
            "Rupture Bottom Depth (km)": [bottom_depths[median_simulation]],
        }
    )
